...
export function timeout() {
    const wait = new Promise((resolve) => { // in constructor Promise you have to enter a execution function
        setTimeout(() => {
            resolve("Timeout!");
        }, 1500);
    });
...
export function xhr() {
    let request = new Promise((resolve, reject) => {
        ...
        xhr.onerror = () => reject("Request Failed"); // only network error, else onload
        xhr.send();
    });
